...
const StylingSettings: React.FC<PropTypes> = ({ ctx, configType }) => {
    ...
    function findDuplicates(items: KeyValuePairType[]): { duplicateIds: number[], duplicateLabels: string[], duplicateValues: string[] } {
      ...
      for (const item of items) {
          idSet.has(item.id) ? duplicateIds.push(item.id) : idSet.add(item.id);
          labelSet.has(item.label) ? duplicateLabels.push(item.label) : labelSet.add(item.label);
          valueSet.has(item.value) ? duplicateValues.push(item.value) : valueSet.add(item.value);
      }
    ...
    const duplicateArrays = () => {
        const { duplicateIds, duplicateLabels, duplicateValues } = findDuplicates(keyValueList);
    ...
    const containsDuplicates = () => {
      ...
      return duplicateIds.length > 0 || duplicateLabels.length > 0 || duplicateValues.length > 0;
    };
